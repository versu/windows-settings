...
$logPath = "$($env:LOG_DIR)\facade.log"
...
try {
    # 実行するスクリプトを順番に定義
    Invoke-ScriptSequentially -ScriptPath "$($env:SCRIPT_FOLDER)\facade-sample5.ps1" -RequireAdmin $false
    Invoke-ScriptSequentially -ScriptPath "$($env:SCRIPT_FOLDER)\facade-sample1.ps1" -RequireAdmin $false
    Invoke-ScriptSequentially -ScriptPath "$($env:SCRIPT_FOLDER)\facade-sample2.ps1" -RequireAdmin $true
    Invoke-ScriptSequentially -ScriptPath "$($env:SCRIPT_FOLDER)\facade-sample4.ps1" -RequireAdmin $true
    Invoke-ScriptSequentially -ScriptPath "$($env:SCRIPT_FOLDER)\facade-sample3.ps1" -RequireAdmin $false
}
catch {
